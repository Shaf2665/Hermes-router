...
```bash
docker exec hermes-router hr agent capabilities
docker exec hermes-router python hermes_agent_runner.py capabilities   # plain image
```

Note that a container's worktree is the container filesystem. Hall must mount the worktree it
owns into the container (`-v /path/to/worktree:/work -w /work`) before running a task there.
